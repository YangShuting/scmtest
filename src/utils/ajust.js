 ...
 */
export const TableBrTitle = props =>
  (props.sort ? <span className="table-title-block sort"><div className="table-title-br"><span>{props.name}</span></div></span> :
  <div className="table-title-br"><span>{props.name}</span></div>);
...
export function HandleTableDataSet(json, item = 'data') {
  const { pagination } = this.state;
  if (json.data) {
    if (pagination) { pagination.total = json.data.total; }
    this.setState({
      pagination,
      loading: false,
      [item]: json.data.rows,
    ...
    this.setState({
      pagination: { current: 1, pageSize: 10, showQuickJumper: true, total: 0, showSizeChanger: true, showTotal: total => `共 ${total} 行` },
      loading: false,
      [item]: [],
    });
  }
 ...
 */
export const TableSetData = (TableParam = { dataName: 'data' }) => (target) => {
  target.prototype.setData = json => HandleTableDataSet.bind(this)(json, TableParam);
  // console.log(setData)

  // Object.assign(target.prototype, ...{setData})
  // target.=true
};
...
export const getColumns = (columns, userInfo, cb) => {
  return columns.map((item, index) => {
    ...
    // switch (key) {
    //     case 'postData':
    //         render = getRenderPostData(item.operation)
    //         break;
    //     case 'editCell':
    //         render = getRenderEditModule(item.operation,userInfo,cb)
    //         break;
    //     case 'valueChange':
    //         render = getValueChange(item.operation)
    //         break;
    //     case 'visibleByKey':
    //         render = getVisibleByKey(item.operation)
    //         break;
    //     case 'openParent' :
    //         render = getReorderGo(item.operation,cb)
    //         break;
    //     default:
    //         break;
    // }
...
export class PostDataInA extends React.Component {
  constructor() {
    ...
    render() {
      ...
      return (
        <a className="tabelCell-action" href="javascript:void(0)" onClick={this.HandelClick}>{item.name}</a>
      );
    }
}
 ...
 * 审核
 */
export const getJudge = (value, row, index) => (value == 1 ? '已审核' : '未审核');
